...
def predict_images():
    ...
    for filepath in filepaths:
        ...
        img = ImageOps.fit(image, (300, 300), Image.Resampling.LANCZOS)
...
btn_select = tk.Button(left_frame, text="Selecione imagens", command=predict_images)
btn_select.pack(pady=20, padx=10)

img_label = tk.Label(left_frame)
img_label.pack(pady=20, padx=20)

result_label = tk.Label(left_frame, text="Resultado: ", font=("Arial", 20))
result_label.pack(pady=20, padx=10)

confidence_label = tk.Label(left_frame, text="Precisão: ", font=("Arial", 20))
confidence_label.pack(pady=20, padx=10)

results = tk.Listbox(root, width=100, height=20)
